...
// --- 1. FIRESTORE LISTENER (DENGAN SKELETON LOADING) ---
...
function renderPagination(totalItems) {
  ...
  const createBtn = (text, target, active = false, disabled = false) => {
    ...
    btn.className = `px-3 py-1 rounded ${active ? "bg-blue-600 text-white" : "bg-gray-200 hover:bg-gray-300"} ${disabled ? "opacity-50 cursor-not-allowed" : ""}`;
  ...
  for (let i = 1; i <= totalPages; i++) pagination.appendChild(createBtn(i, i, i === currentPage));
...
// --- 4. LOGIKA MODAL (CLOSE BUTTONS) ---
function closeDetail() { detailModal.classList.add("hidden"); detailModal.classList.remove("flex"); }
function closeTambah() { tambahModal.classList.add("hidden"); tambahModal.classList.remove("flex"); }
...
// --- 5. LOGIKA KOLASE (PILIH GAMBAR & DOWNLOAD) ---
...
function openFotoSelectModal(item) {
  ...
  options.innerHTML = `
    <label class="flex flex-col items-center cursor-pointer"><input type="radio" name="fotoChoice" value="foto1" checked><img src="${item.foto1}" class="w-24 h-24 object-cover rounded mt-1"></label>
    <label class="flex flex-col items-center cursor-pointer"><input type="radio" name="fotoChoice" value="foto2"><img src="${item.foto2}" class="w-24 h-24 object-cover rounded mt-1"></label>
  `;
...
document.getElementById("fotoSelectCancel").onclick = () => document.getElementById("fotoSelectModal").classList.add("hidden");
...
function updateOrderBadges() {
  selectedItems.forEach((item, index) => {
    ...
    if (targetDiv) targetDiv.querySelector(".orderBadge").textContent = index + 1;
  });
}

// --- 6. DOWNLOAD KOLASE (HTML2CANVAS) ---
document.getElementById("buatKolaseBtn").addEventListener("click", () => {
  ...
  kolasePreview.className = "grid grid-cols-2 grid-rows-2 w-full h-[400px] gap-0 overflow-hidden";
...
// Delegasi klik tabel (Hapus & Detail)
...
setInterval(() => {
  const n = new Date();
  if (n.getHours() === 21 && n.getMinutes() === 0) {
    const b = barangData.filter(i => !i.verifikasi);
    if (b.length > 0) alert(`Reminder: ${b.length} barang belum diverifikasi!`);
  }
}, 60000);
